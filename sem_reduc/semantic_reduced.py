...
def main(args):
    ...
    # Scale the epochs based on the reduction so that the number of optimization iterations remains the same
    args.epochs = int(round(args.epochs / args.reduction))
    ...
    # Create and load the model that will be used to create the cluster reps
    # cluster_model = torchvision.models.resnet50(num_classes=4)
    # ckpt = torch.load('53524196-best-weights.pth')
    # cluster_model.load_state_dict(ckpt['model_state_dict'])
    # cluster_model.fc = misc.Null()
    # cluster_model.cuda()
    # cluster_model.eval()
    ...
    # )
    split='train'
    train_dataset = data.SemanticReducedDataset(
        os.path.join(args.data_dir, split),
        args.reduction,
        model=None,
        transform=train_transforms,
        rep_transform=test_transforms,
        repr_cache=os.path.join(args.cache_dir, f'marco_{split}_repr.dat'),
        repr_load_cache=True,
        reduc_cache=os.path.join(args.cache_dir, f'marco_{split}_{args.reduction}.dat'),
    ...
    for epoch in range(args.epochs):
        ...
        with torch.no_grad():
            ...
            marco_eval_loss = eval_loss / len(marco_test_dataset)
            marco_eval_acc = eval_acc / len(marco_test_dataset)
            
            if args.is_master:
                print(f'Marco Evalutation: Loss {round(marco_eval_loss.item(), 4)}, Acc {round(marco_eval_acc.item(), 4)}')
            ...
            #     for j in range(preds.shape[0]):
            #         conf_matrix[preds[j]][target[j]] += 1a

            #     eval_losses.append(loss.item())
            #     eval_acc.append(acc.item())

            # c3_eval_loss = np.mean(eval_losses)
            # c3_eval_acc = np.mean(eval_acc)
        ...
        if args.is_master:
            ...
            # plt.plot(c3_eval_losses)

            axes.plot(marco_eval_accs)
            # plt.plot(c3_eval_accs)
            axes.legend(['Marco Train Loss', 'Marco Train Acc', 'Marco Test Loss', 'Marco Test Acc'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_dir',type=str, default='/scratch1/ros282/marco_full')
    parser.add_argument('--arch', type=str, default='resnet50')
    parser.add_argument('--cache_dir', type=str, default='/scratch1/ros282/cache')
