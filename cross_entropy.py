...
def main(args):
    ...
    with torch.no_grad():
        for i in tqdm.tqdm(range(len(dataloader))):
            ...
            preds = F.log_softmax(model(imgs.cuda()), dim=-1)
            loss = F.cross_entropy(preds, labels.cuda(), reduction='none')

            loss_lst.append(loss)
            
    loss_lst = torch.cat(loss_lst)
    loss_lst = [x.item() for x in loss_lst]
    file_paths = [x[0] for x in dataset.samples]


     
    with open('cross_entropy.json', 'w') as fp:
        json.dump(
            list(zip(loss_lst,file_paths)),
            fp,
            indent=4
        )
