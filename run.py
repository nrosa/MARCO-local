...
Adapted from timm (https://github.com/rwightman)
...
parser.add_argument('--local_eval_data_dir', metavar='DIR',
...
# Args for adding local images
parser.add_argument('--add_local_images', action='store_true')
...
parser.add_argument('--image_cnt', type=int, default=1000)
...
def main():
    ...
    if args.model == 'modresnetv2':
        model = timm.models.resnetv2.ResNetV2(
            args.layers,
            channels=args.channels,
            num_classes=args.num_classes,
            drop_rate=args.drop,
            drop_path_rate=0. if args.drop_path is None else args.drop_path,
            global_pool='avg' if args.gp is None else args.gp,
        )
    ...
    local_eval = args.local_eval_data_dir is not None
    ...
    dataset_eval = create_dataset(
        ...
        batch_size=args.batch_size)
    if local_eval:
        dataset_local_eval = create_dataset(
            args.dataset, root=args.local_eval_data_dir, search_split=False, is_training=False,
            class_map=args.class_map,
            download=args.dataset_download,
            batch_size=args.batch_size)
    ...
    if local_eval:
        loader_local_eval = create_loader(
            dataset_local_eval,
            input_size=data_config['input_size'],
            batch_size=args.validation_batch_size or args.batch_size,
            is_training=False,
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            if local_eval:
                local_metrics = validate(model, loader_local_eval, validate_loss_fn, args, amp_autocast=amp_autocast)
                for key in local_metrics:
                    eval_metrics['local_'+key] = local_metrics[key]
                ...
                if model_ema is not None and not args.model_ema_force_cpu:
                    ...
                    local_ema_metrics = validate(
                        model_ema.module, loader_local_eval, validate_loss_fn, args, amp_autocast=amp_autocast, log_suffix=' (EMA)')
                    eval_metrics = ema_eval_metrics
                    for key in local_ema_metrics:
                        eval_metrics['local_'+key] = local_ema_metrics[key]
